...
const Religion = ({ religionData, toggleHandler, handleChangeData }) => {
  const [state1, setState1] = useState(-1);
  const [curIndex, setCurIndex2] = useState(-1);
  const [expanded, setExpanded] = React.useState("panel1");
  const [skilldup, setSkillDup] = useState(religionData);
  const [skillName, setskillName] = useState("");

  const searchWorkPlan = () => {
    setSkillDup(religionData.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setskillName("");
    setSkillDup(religionData);
  };
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: "Search & Select : Religion",
      open: "true",
