...
const SubDepertment = ({
  ...
  const [skilldup, setSkillDup] = useState(subDepartmentArr);
  const [skillName, setskillName] = useState("");

  const searchWorkPlan = () => {
    setSkillDup(subDepartmentArr.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setskillName("");
    setSkillDup(subDepartmentArr);
  };
  const filterStaff = (item) => {
    if (skillName != "") {
      if (
        item?.subDepartmentName.toLowerCase().includes(skillName.toLowerCase())
      ) {
        return item;
      }
    } else {
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: "Search & Select : Sub Department",
      open: "true",
