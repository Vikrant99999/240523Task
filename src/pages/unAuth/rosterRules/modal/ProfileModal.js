...
const ProfileModal = ({
  ...
  const [profiledup, setProfileDup] = useState(profile);
  const [profileName, setProfileName] = useState("");

  const searchWorkPlan = () => {
    setProfileDup(profile.filter(filterStaff));
  };
  ...
  const filterStaff = (item) => {
    if (profileName != "") {
      if (item?.profileName.toLowerCase().includes(profileName.toLowerCase())) {
        return item;
      }
    } else {
      return item;
