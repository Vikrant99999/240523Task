...
const NationalityModal = ({ citizenData, toggleHandler, handleChangeData }) => {
  const [state1, setState1] = useState(-1);
  const [curIndex, setCurIndex2] = useState(-1);
  const [expanded, setExpanded] = React.useState("panel1");
  const [skilldup, setSkillDup] = useState(citizenData);
  const [skillName, setskillName] = useState("");

  const searchWorkPlan = () => {
    setSkillDup(citizenData.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setskillName("");
    setSkillDup(citizenData);
  };
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: "Search & Select : Nationality",
      open: "true",
