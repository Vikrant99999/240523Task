...
export const DepartmentFilterModal = (props) => {
  ...
  useEffect(() => {
    var localArray = [];
    console.log(oriPagedata, "orio")
    if (oriPagedata) {
      if (employeeupdateData.length > 0) {
        console.log("1");
        localArray = oriPagedata.map((item) => ({
          ...item,
          checked: employeeupdateData.includes(
  ...
  const filterclickhandler = () => {
    var localArray = filterData
      ...
      .map((option) => option.department || option.departmentName);
