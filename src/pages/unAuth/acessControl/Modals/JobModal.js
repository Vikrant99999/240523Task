...
const JobModal = ({ jobTitle, toggleHandler, handleChangeData }) => {
  const [state1, setState1] = useState(-1);
  const [curIndex, setCurIndex2] = useState(-1);
  const [expanded, setExpanded] = React.useState("panel1");
  const [jobTitleDup, setJobTitleDup] = useState(jobTitle);
  const [jobTitleName, setJobTitleName] = useState("");

  const searchWorkPlan = () => {
    setJobTitleDup(jobTitle.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setJobTitleName("");
    setJobTitleDup(jobTitle);
  };
  const filterStaff = (item) => {
    if (jobTitle != "") {
      if (item?.jobTitle.toLowerCase().includes(jobTitleName.toLowerCase())) {
        return item;
      }
    } else {
      return item;
