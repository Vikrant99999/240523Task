...
import { workLocation } from "../../../../services/api";
...
const WorkLocationModal = ({
  ...
  const [workLocationdup, setWorkLocationDup] = useState(workLocationArr);
  const [workLocationName, setWorkLocationName] = useState("");

  const searchWorkPlan = () => {
    setWorkLocationDup(workLocationArr.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setWorkLocationName("");
    setWorkLocationDup(workLocationArr);
  };
  const filterStaff = (item) => {
    if (workLocationName != "") {
      if (
        item?.locationName
          .toLowerCase()
          .includes(workLocationName.toLowerCase())
      ) {
        return item;
      }
    } else {
  ...
  const empChange = (e) => {
    setWorkLocationName(e.target.value);
  };
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: "Search & Select : Work Location",
      open: "true",
