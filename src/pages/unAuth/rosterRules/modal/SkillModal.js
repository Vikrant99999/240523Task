...
const SkillModal = ({ skillData, toggleHandler, handleChangeData }) => {
  const [state1, setState1] = useState(-1);
  const [curIndex, setCurIndex2] = useState(-1);
  const [expanded, setExpanded] = React.useState("panel1");
  const [skilldup, setSkillDup] = useState(skillData);
  const [skillName, setskillName] = useState("");

  const searchWorkPlan = () => {
    setSkillDup(skillData.filter(filterStaff));
  };

  const resetWorkPlan = () => {
    setskillName("");
    setSkillDup(skillData);
  };
  const filterStaff = (item) => {
    if (skillName != "") {
      if (item?.skill.toLowerCase().includes(skillName.toLowerCase())) {
        return item;
      }
    } else {
      return item;
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: "Search & Select : Skill",
      open: "true",
