...
const RotaPopup2 = (props) => {
  const { toggleHandler, rotaLov, handleChangeRota, state1, setState1, value } =
    props;
  const [staffdup, setStaff] = useState(rotaLov);
  const [empname, setEmpname] = useState("");
  // console.log(staffdup, "staffdup");
...
console.log(rotaLov,"rotaLov");
  ...
  const filterStaff = (item) => {
    ...
    if (empname != "") {
      if (item?.workDurationCode.toLowerCase().includes(empname.toLowerCase())) {
        return item;
      }
    } else {
      return item;
  ...
  const data = {
    ...
    customDialogProps: {
      maxWidth: "sm",
      dialogTitle: `Search & Select : ${value}`,
      open: "true",
